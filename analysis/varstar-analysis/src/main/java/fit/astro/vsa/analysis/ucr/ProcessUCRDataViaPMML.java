 ...
 */
public class ProcessUCRDataViaPMML extends ProcessTimeDomainViaPMML {

    private static final Random RAND = new Random(42L);

    private static final Logger LOGGER
            = LoggerFactory.getLogger(ProcessUCRDataViaPMML.class);
     ...
     */
    public static void execute(double tau, double mu, double lambda, boolean isTest, 
        ...
        // =================================================================
        if (!isTest) {
            trainPMML(tau, mu, lambda, "UCR");
        } else {
            testPMML(19, tau, mu, lambda,  "UCR");
        }
    ...
    private static void generatePatterns(TimeDomainAttributeMaps trainingData,
        ...
        for (Integer idx : trainTest.getTrainingData()) {
            ...
            setOfPatterns_Training.get(idx).put(ReadingInLINEARData.Views.SSMM.toString(),
                    canonicalVariates_SSMM.getCanonicalVariates().get(idx));
            setOfPatterns_Training.get(idx).put(ReadingInLINEARData.Views.DF.toString(),
                    canonicalVariates_DF.getCanonicalVariates().get(idx));
