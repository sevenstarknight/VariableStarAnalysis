 ...
 */
public class TrainCrossTestGenerator {
    ...
    private final List<Integer> testingData;

    /**
     * 5-fold cross val
     *
     * @param setOfClasses
     * @param fractionIsTesting
     * @param rand
     */
    public TrainCrossTestGenerator(
            Map<Integer, String> setOfClasses,
            double fractionIsTesting, Random rand) {
        ...
        for (String label : classMembers.keySet()) {
            ...
            tempList.stream().forEach((idx) -> {
                if (rand.nextDouble() > fractionIsTesting) {
     ...
     */
    public List<Integer> getTestingData() {
        return testingData;
    }
