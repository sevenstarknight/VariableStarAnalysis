 ...
 */
public class KNNMultiMatrixMetric {

    // ============================================
    // Input
    private final Map<Integer, Map<String, RealMatrix>> setOfTrainingData;
     ...
     */
    public KNNMultiMatrixMetric(
            Map<Integer, Map<String, RealMatrix>> setOfTrainingData,
    ...
    public ClassificationResult execute(int kValue,
            String missedLabel, Map<Integer, Map<String, RealMatrix>> inputPatternMap) {

        // Initialization
        Integer tmp = (new ArrayList<>(inputPatternMap.keySet())).get(0);
        Map<String, RealMatrix> patterns = inputPatternMap.get(tmp);

        Map<String, MultiViewMetric_MV> multiMatrix = new HashMap<>(patterns.size());

        isEuclidean = Boolean.TRUE;

        for (String feature : patterns.keySet()) {
            RealMatrix uk = MatrixUtils.createRealIdentityMatrix(
                    patterns.get(feature).getColumnDimension());

            RealMatrix vk = MatrixUtils.createRealIdentityMatrix(
                    patterns.get(feature).getRowDimension());

            MultiViewMetric_MV mvv = new MultiViewMetric_MV(uk, vk,
    ...
    public ClassificationResult execute(int kValue, String missedLabel,
            Map<String, MultiViewMetric_MV> multiMatrix,
            Map<Integer, Map<String, RealMatrix>> inputPatternMap) {

        MultiViewMetricDistance_MV metricDistance
                = new MultiViewMetricDistance_MV(multiMatrix);

        Map<Integer, String> labelEstimate = new HashMap<>(inputPatternMap.keySet().size());
        Map<Integer, Map<String, Double>> labelPostProb = new HashMap<>(inputPatternMap.keySet().size());
        ...
        for (Integer idx : inputPatternMap.keySet()) {
            ...
            Map<String, RealMatrix> crossvalPattern = inputPatternMap.get(idx);
                ...
                if (isEuclidean) {
                    ...
                    for (String view : multiMatrix.keySet()) {
                        distance += (setOfTrainingData.get(trainingDataID).get(view)
                                .subtract(crossvalPattern.get(view))).getFrobeniusNorm();
                    }
    ...
    public Map<Integer, ClassificationResult> execute(int[] kValues,
        ...
        Map<String, MultiViewMetric_MV> multiMatrix = new HashMap<>();
