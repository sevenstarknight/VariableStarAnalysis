...
public class KNNMatrixMetric {
     ...
     */
    public KNNMatrixMetric(
            Map<Integer, RealMatrix> setOfTrainingData,
    ...
    public ClassificationResult execute(int kValue,
            String missedLabel, Map<Integer, RealMatrix> inputPatternMap) {

        RealMatrix firstMatrix
                = setOfTrainingData.values().iterator().next();

        isEuclidean = Boolean.TRUE;

        RealMatrix metricMatrixA = MatrixUtils.createRealIdentityMatrix(
                firstMatrix.getRowDimension());

        RealMatrix metricMatrixB = MatrixUtils.createRealIdentityMatrix(
                firstMatrix.getColumnDimension());

        RealMatrix[] metricMatrix = new RealMatrix[]{metricMatrixA, metricMatrixB};
    ...
    public ClassificationResult execute(int kValue, String missedLabel,
            RealMatrix[] metricMatrix, Map<Integer, RealMatrix> inputPatternMap) {

        MetricDistance_MV metricDistance = new MetricDistance_MV(metricMatrix);
        ...
        for (Integer idx : inputPatternMap.keySet()) {
            ...
            RealMatrix crossvalPattern = inputPatternMap.get(idx);
            ...
            for (Integer trainingDataID : setOfTrainingData.keySet()) {
                ...
                Double distance;
                if (isEuclidean) {
                    distance = (setOfTrainingData.get(trainingDataID)
                            .subtract(crossvalPattern)).getFrobeniusNorm();
                } else {
                    distance = metricDistance.matrixDistance(
